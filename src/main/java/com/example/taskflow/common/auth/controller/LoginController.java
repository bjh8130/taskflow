...
public class LoginController {
    ...
    // 로그인
    @PostMapping("/login")
    public ResponseEntity<GlobalResponse<LoginResponseDto>>loginApi(@Valid @RequestBody LoginRequestDto requestDto) {

        LoginResponseDto result = loginService.login(requestDto);
        return ResponseEntity.status(HttpStatus.OK).body(GlobalResponse.success(true,"로그인 성공",result));
    }
